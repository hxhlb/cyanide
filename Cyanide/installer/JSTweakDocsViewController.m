...
- (void)buildTweakDocsInStack:(UIStackView *)stack
{
    ...
    [stack addArrangedSubview:[self docCard:@"Parameter Declaration"
        body:@"Add @param comments at the top of your script. Cyanide parses them and generates native UI controls in Settings.\n\n"
              ...
              "color — Hex color picker (#RRGGBB or #RRGGBBAA)\n"
              "// @param: color | tint | Tint Color | #00FFCC80\n\n"
    ...
    [stack addArrangedSubview:[self docCard:@"Cleanup"
        body:@"Define an optional global cleanup() function for scripts that change in-memory state:\n\n"
              "globalThis.cleanup = function () {\n"
              "  r_msg2_main(view, \"setHidden:\", 0);\n"
              "  clearInterval(timer);\n"
              "};\n\n"
              "Cyanide calls cleanup() before it stops a running script during Clean Up, package disable/removal, or session shutdown. Restore only state captured by the script before it changed it. Keep cleanup() synchronous and fast. Persistent file or system changes cannot be restored automatically."]];
...
- (NSString *)tweakMarkdown
{
    ...
    "| `color` | `// @param: color \\| tint \\| Color \\| #00FFCC80` |\n"
